...
{
	int restAfteBuy = -prices[0], acumProfit = 0;
	int delayAcumProfit = 0;
	
	for (int i = 1; i < prices.size(); ++i)
	{
		int prevRest = restAfteBuy;
		int prevAcumProfit = acumProfit;
		
		// Point of purchase. The remaining money after a new purchase,
		// the lower the price - the larger the balance
		restAfteBuy = std::max(prevRest, delayAcumProfit - prices[i]);
		...
		delayAcumProfit = std::max(delayAcumProfit, prevAcumProfit); // maximum between
	}
